...
$(document).ready(function () {
	...
	$(document).on("click", ".popover-btn", function (e) {
		...
		if (id != null && id != undefined && helpObj != null && helpObj != undefined) {
			var content =
				...
				'<div id="spin' +
				id +
				'" class="innerloader popup-spinner spinner spinner--black">' +
			...
			$(this)
				.popover({
					...
					trigger: "focus",
					template:
						'<div class="popover iftdssHelpPopover" role="tooltip"><div class="arrow"></div><h3 class="popover-title"></h3><div class="popover-content single"></div></div>'
				})
				.popover("show");
	...
	$(document).on("click", "#startTour", function (e) {
		...
		if (steps.length) {
			var tour = new Tour({
				...
				template:
					'<div class="popover iftdssHelpPopover" role="tooltip"> <div class="arrow"></div> <h3 class="popover-title"></h3> <div class="popover-content"></div> <div class="popover-navigation"> <div class="btn-group"> <button class="btn btn-sm btn-default" data-role="prev">&laquo; Prev</button> <button class="btn btn-sm btn-default" data-role="next">Next &raquo;</button> <button class="btn btn-sm btn-default" data-role="pause-resume" data-pause-text="Pause" data-resume-text="Resume">Pause</button> </div> <button class="btn btn-sm btn-default" data-role="end">End tour</button> </div> </div>'
				// debug:true,
			});
			tour.init();
			tour.start();
	...
	if (window.location.href.includes("iftdssdev.firenet.gov")) {
		PopOverHelpers.init("https://iftdssdev.firenet.gov/firenetHelp/help/pageHelp");
	} else if (window.location.href.includes("iftdss.firenet.gov")) {
		PopOverHelpers.init("https://iftdss.firenet.gov/firenetHelp/help/pageHelp");
	} else if (window.location.href.includes("iftdssqa.firenet.gov")) {
		PopOverHelpers.init("https://iftdssqa.firenet.gov/firenetHelp/help/pageHelp");
	} else {
		PopOverHelpers.init("http://localhost:8080/firenetHelp/help/pageHelp_qa");
	}
	// https://iftdss.firenet.gov/firenetHelp/help/pageHelp/content/30-tasks/lndscpedit/landscapeeditabout.htm#EditPickaLandscape
});
...
var PopOverHelpers = {
	...
	getPathFull: function (id) {
		if (PopOverHelpers.list[id]) {
			return PopOverHelpers.root + PopOverHelpers.list[id].location;
		} else {
			return "";
		}
	}
